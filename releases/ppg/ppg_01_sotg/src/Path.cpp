...
int Path::loadPath(char *filename) {
	FILE *handle;
	char s[255],buf[255];
	...
	if(filename!=NULL) {
		...
		if(strncmp (buf, "*3DSMAX_ASCIIEXPORT", strlen("*3DSMAX_ASCIIEXPORT")) != 0) {
		...
		while(!feof(handle)) {
			fgets(buf,255,handle);
			//printf("%s",buf);
			if(strncmp(buf, "\t*SHAPE_LINECOUNT 1", strlen("\t*SHAPE_LINECOUNT 1"))==0) { 
