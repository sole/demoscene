#include "ResourceOSStarField.h"
#include <iostream>
#include <cmath>
using namespace std;
#include "Render.h"

CResourceOSStarField::CResourceOSStarField(){
	m_numParticles = 200;
	m_origin1.x=-10;
	m_origin1.y=100;
	m_origin1.z=-10;
...
inline void CResourceOSStarField::initParticle(CPoint *p) {
...
inline void CResourceOSStarField::drawParticle(float x2, float y2, float z2, float dens, CPoint vup, CPoint vright) {
...
void CResourceOSStarField::play(float _time) {	
	...
	myCam.SetFov(30);
	myCam.Place(CPoint(0,-1,0), CPoint(0,100,0), CPoint(1,0,0));
	myCam.toOGL();
...
void CResourceOSStarField::init() {
	int textureIndex;
	...
	for(i=0; i<m_numParticles; i++) {
		...
		initParticle(&m_particlesList[i+1]);
	}
}

void CResourceOSStarField::start() {
	
}

void CResourceOSStarField::deInit(void) {
	
}

string CResourceOSStarField::getType(void) {
	return "CResourceOSStarField";
}

void CResourceOSStarField::setParam(string name, string value) {
	if(name=="r") {
