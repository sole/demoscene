...
/*
TODO
- Lfo para bajar la frequency rapido etc
- hacer fcn independiente del tamanyo del buffer (no rellenar siempre la onda entera o algo asi) || t=(v->pos+i)*invSAMPLE_RATE;
- distort
- delay | reverb asi rapidito
- compressor (??)
- usar volumen en la nota
- efectos (cambiar parametros?)
...
{
	unsigned char note;
	float volume; //unsigned char volume;
...
extern int sorollet_get_buffer(float *buffer, int position, int num_samples);
...
typedef void (*osc1_function_ptr)(float*, int, float, int, int);
