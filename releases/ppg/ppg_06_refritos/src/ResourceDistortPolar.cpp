...
CResourceDistortPolar::CResourceDistortPolar() {
...
void CResourceDistortPolar::play(float _time) {	
	...
	static float xv[RESOURCEDISTORTPOLAR_MAX_X], yv[RESOURCEDISTORTPOLAR_MAX_Y]; // x and y vertexes
	static float txv[RESOURCEDISTORTPOLAR_MAX_X], tyv[RESOURCEDISTORTPOLAR_MAX_Y]; // tex x and y vertexes
	...
	float alpha, tempx, veclength;
	for(i=0; i < m_numX+1; i++) {
		for(j=0; j < m_numY+1; j++) {
			if( abs(xv[i]) < m_radioX) {
				// first get the angle of the vector of current point to centre and the X axis:
				// PC < X
				if(xv[i]!=0) {
					tempx = 0.5;
				} else {
				...
				alpha = atan(yv[j] / tempx);
				
				// new x and y values...
				veclength = sqrt( xv[i]*xv[i] + yv[j]*yv[j]);
				xv[i] = veclength * cos(alpha+_time*m_periodX);
				yv[j] = veclength * sin(alpha+_time*m_periodY);
			}
...
void CResourceDistortPolar::init() {
...
void CResourceDistortPolar::start() {
	
}

void CResourceDistortPolar::deInit(void) {
	
}

string CResourceDistortPolar::getType(void) {
	return "CResourceDistortPolar";
}

void CResourceDistortPolar::setParam(string name, string value) {
	if(name=="r") {
	...
	if(name=="num_x") {
		...
		this->m_numX = (tmp < RESOURCEDISTORTPOLAR_MAX_X-1 ? tmp : RESOURCEDISTORTPOLAR_MAX_X);
	}
	if(name=="num_y") {
		unsigned int tmp = (unsigned int) atoi(value.c_str());
		this->m_numY = (tmp < RESOURCEDISTORTPOLAR_MAX_Y-1 ? tmp : RESOURCEDISTORTPOLAR_MAX_Y);
	}
	if(name=="radio_x") {
