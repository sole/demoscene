...
//	float acum;
	...
	/* Recordatorio de la multiplicacion vector*matriz
	   ...
	   Se multiplica el vector por cada columna de la matriz:
...
Punto Punto::homogeneo()					
{
	...
	else return Punto(x/w,y/w,z/w,1.0f);
}
...
{
	if(i<4 && i>=0) return Punto(e[i][0],e[i][1],e[i][2],e[i][3]);
	else {		
		exit(1);
	};
	return Punto();
...
{
	if(i<4 && i>=0) return Punto(e[0][i],e[1][i],e[2][i],e[3][i]);
	else {		
		exit(1);
	};
	return Punto();
...
//Acumula la traslacion propuesta
{
	...
	Matriz::operator=((*this)*t); // uso el operador = de la clase papi
...
{  
	...
	Matriz::operator =((*this)*ms); //un poco marciano no???

	// Se deshace la traslacion!
...
//Acumula el giro alrededor de un eje generico
{
	...
	float lat= vectorEje.latitud();		//latitud
	float lon= vectorEje.longitud();		//longitud
...
//Acumula la traslate propuesta
{
	...
	for (i=0;i<3;i++) t.e[i][3]=d[i];
	CMatrix::operator=((*this)*t); // uso el operador = de la clase padre
