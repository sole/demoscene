...
CResourceBoxBlur::CResourceBoxBlur() {
...
void CResourceBoxBlur::play(float _time) {	
	...
	float texx=texCoordX, texy=texCoordY;
	float incx=0, incy=0;
	float offset = 1;
	...
	for(int i=0; i<this->m_numCopies; i++) {
		...
		currColor.w = currAlpha * 0.5;
		m_Render->setColor(currColor);
		
		m_Render->setTextureCoord(0,0);
		m_Render->addVertex(-width-incx-offset,-height-incy);
		
		m_Render->setTextureCoord(texx,0);
		m_Render->addVertex(width+incx-offset,-height-incy);
		
		m_Render->setTextureCoord(texx,texy);
		m_Render->addVertex(width+incx-offset, height+incy);
		
		m_Render->setTextureCoord(0,texy);
		m_Render->addVertex(-width-incx-offset,height+incy);
		
		
		currColor.w = currAlpha * 0.3;
		m_Render->setColor(currColor);
		...
		m_Render->setTextureCoord(0,0);
		m_Render->addVertex(-width-incx,-height-incy-offset);
...
void CResourceBoxBlur::start() {
	
}

void CResourceBoxBlur::deInit(void) {
	
}

string CResourceBoxBlur::getType(void) {
	return "CResourceBoxBlur";
}

void CResourceBoxBlur::setParam(string name, string value) {
	if(name=="r") {
