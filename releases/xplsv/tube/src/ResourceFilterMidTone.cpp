...
using namespace std;
CResourceFilterMidTone::CResourceFilterMidTone(){
	this->m_numVertex = 32;
	m_numX = 1;
	m_numY = 1;
...
void CResourceFilterMidTone::play(float _time) {	
...
void CResourceFilterMidTone::init() {
...
void CResourceFilterMidTone::start() {
	
}

void CResourceFilterMidTone::deInit(void) {
	delete []this->m_framebuffer;
}

string CResourceFilterMidTone::getType(void) {
	return "CResourceFilterMidTone";
}

void CResourceFilterMidTone::setParam(string name, string value) {
	if(name=="clear") {
	...
	if(name=="num_x") {
		...
		this->m_numX = (tmp < RESOURCEFILTERMIDTONE_MAX_X-1 ? tmp : RESOURCEFILTERMIDTONE_MAX_X);
	}
	if(name=="num_y") {
		unsigned int tmp = (unsigned int) atoi(value.c_str());
		this->m_numY = (tmp < RESOURCEFILTERMIDTONE_MAX_Y-1 ? tmp : RESOURCEFILTERMIDTONE_MAX_Y);
	}
	if(name=="blending") {
